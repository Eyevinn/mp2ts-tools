...
func parseOptions() common.Options {
	opts := common.Options{ShowStreamInfo: true, ShowService: false, ShowPS: true, ShowNALU: false, ShowSEI: false, ShowStatistics: false}
...
func parsePSInfo(ctx context.Context, w io.Writer, f io.Reader, o common.Options) error {
	return avc.ParseAll(ctx, w, f, o)
}

func main() {
	o, inFile := common.ParseParams(parseOptions)
	err := common.Execute(os.Stdout, o, inFile, parsePSInfo)
	if err != nil {
		log.Fatal(err)
	}
}
